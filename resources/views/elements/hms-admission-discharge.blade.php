...
function findDueandChange(grand_total,paid_amount){
    var due   = parseFloat(grand_total).toFixed(2) - parseFloat(paid_amount).toFixed(2);
    ...
    if(grand_total > 0){
        ...
        var diagonDue = parseFloat($('.diagonTotalDue').text())||0;
        var pharmaDue = parseFloat($('.pharmatotalDue').text())||0;
        var operationDue = parseFloat($('.operationTotalDue').text())||0;
        var emergenDue = parseFloat($('.emergencyTotalDue').text())||0;
        var bedTotalDue = parseFloat($('.bedTotalDue').text())||0;
        var admissionTotalDue = parseFloat($('.admissionTotalDue').text())||0;
        var sum = due + diagonDue + pharmaDue+ operationDue +emergenDue + bedTotalDue + admissionTotalDue;
        if(sum === 0 ){
...
//table search
    ...
    $("#bed_id").change(function() {
        ...
        if(sub_total > 0){
            var bed = parseFloat($('#bed_id').val())||0;
            if(bed != 0){
                $('#finalCheckOUT').removeAttr("disabled");
            }
        }
    ...
    //     var bed_no = $(this).find(':selected').data("bed_no");

    //     var html;
    //     html +=`<td class="text-center sl"></td>`;
    //     html +=`<td>Admission into ${bed_no}</td>`;
    //     html +=`<td class="text-right unit_price">${price}.00</td>`;
    //     html +=`<td align="center">`;
    //     html +=`<input type="hiden" class="display_none" name="test_items[]" value="${1}">`;
    //     html +=`<input type="hiden" class="display_none" name="test_item_price[]" value="${price}">`;
    //     html +=`</td>`;
    //     $('#CartRowId01').html(html);
    //     // $('#rowId'+1).hide();
    //     $('#finalCheckOUT').removeAttr("disabled");
    //     CheckOutCalculation();
    //     serialPOPup();

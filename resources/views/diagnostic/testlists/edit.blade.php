...
@component('layout.inc.breadcrumb')
    @slot('title')
       {{__('messages.update')}} {{trans_choice('messages.test_list',10)}}
    @endslot
@endcomponent
        ...
        <div class="col-lg-12 col-md-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <h6 class="card-subtitle">{{__('messages.update')}} {{trans_choice('messages.test_list',10)}}</h6>
                    <hr class="hr-borderd">
                    <form class="form-material m-t-40 form" action="{{ url('/diagnostic/testlists/'.$DiagonTestList->id) }}" method="post">
                        @csrf
                        @method('PUT')
                        <div class="form-group row {{ $errors->has('name') ? ' has-danger' : '' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="text" name="name" value="{{old('name',$DiagonTestList->name)}}" class="form-control" id="name" placeholder="Test Name" required autocomplete="off">
                        ...
                        <div class="form-group row {{ $errors->has('test_category_id') ? ' has-danger' : '' }}">
                            ...
                            <div class="col-sm-10">
                                <select name="test_category_id" class="form-control" id="test_category_id" autocomplete="off">
                                 <option value="">Select Test Category</option>
                                 @foreach ($diagonTestCategories as $row)
                                 <option value="{{ $row->id}}" @if($row->id==$DiagonTestList->test_category_id) selected @endif>{{ $row->category }}</option>
                                 @endforeach   
                                </select>
                        ...
                        <div class="form-group row {{ $errors->has('price') ? ' has-danger' : '' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="text" name="price" value="{{old('price',$DiagonTestList->price)}}" class="form-control" id="price" placeholder="Price" required autocomplete="off">

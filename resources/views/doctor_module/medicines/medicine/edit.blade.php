        ...
        <div class="col-lg-12 col-md-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <h6 class="card-subtitle">Update Medicine</h6>
                    <hr class="hr-borderd">
                    <form class="form-material m-t-40 form" action="{{ url('medicines/'.$medicine->id) }}" method="post">
                        @csrf
                        @method('PUT')
                        <div class="form-group row {{ $errors->has('name') ? ' has-danger' : '' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="text" name="name" value="{{old('name',$medicine->name)}}" class="form-control" id="name" placeholder="Service Name" required autocomplete="off">
                        ...
                        <div class="form-group row {{ $errors->has('pre_medicine_type_id') ? ' has-danger' : '' }}">
                            ...
                            <div class="col-sm-10">
                                <select name="pre_medicine_type_id" class="form-control" id="pre_medicine_type_id" required autocomplete="off">
                                 ...
                                 @foreach ($type as $row)
                                 <option value="{{ $row->id}}"@if($row->id==$medicine->pre_medicine_type_id) selected @endif>{{ $row->name }}</option>
                                 @endforeach
                                </select>
                        ...
                        <div class="form-group m-b-0 float-right">
                            <a href="{{url('medicines/')}}" class="btn bg-theme text-white">{{__('messages.back')}}</a>

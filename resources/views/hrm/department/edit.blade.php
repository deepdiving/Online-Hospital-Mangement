    ...
    <div class="row">
        <div class="col-lg-12 col-md-12">
            <div class="card">
                ...
                <h6 class="card-subtitle">Update New Department</h6>
                <hr class="hr-borderd">
                    <form class="form-material m-t-40 form" action="{{ url('department/'.$department->id) }}" method="post">
                        @method('put')
                        @csrf
                        <div class="form-group row {{ $errors->has('name') ? ' has-danger' : '' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="text" name="name" value="{{old('name',$department->name)}}" class="form-control" id="name" required autocomplete="off">
                        ...
                        <div class="form-group row {{ $errors->has('description') ? ' has-danger' : '' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="text" name="description" value="{{old('description',$department->description)}}" class="form-control" id="name" required autocomplete="off">
                                @include('elements.feedback',['field' => 'description'])
                            </div>
                        </div>
                        <div class="form-group m-b-0 float-right">
                            <a href="{{url('department/')}}" class="btn bg-theme text-white">{{__('messages.back')}}</a>
